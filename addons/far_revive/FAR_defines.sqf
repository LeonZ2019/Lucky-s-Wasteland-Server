...
#define IS_MEDIC(UNIT) ((FAR_ReviveMode > 0 || {IS_MEDICAL_VEHICLE(UNIT)}) && (FAR_ReviveMode != 2 || {"Medikit" in items UNIT || ((UNIT getVariable "mf_inventory_list") select {(_x select 0) == "defibrillator"}) select 0 select 1 > 0 || {IS_MEDICAL_VEHICLE(UNIT) && unitIsUAV UNIT}}))
