...
while {true} do
{
	...
	} else
	{
		...
		if (alive _currUAV) then
		{
			...
			{
				if !(_currUAV getVariable ["A3W_vehicleVariant", ""] in ["greyhawkUnarmed", "falconUnarmed"] && ["_soldier_UAV_", typeOf player] call fn_findString != -1 && _uavRoleEnabled findIf {_currUAV isKindOf _x} != -1) then
				{
					_role = UAVControl _currUAV select 1;
					if (_role != "") then
					{
						LAYER cutText ["THERMAL IMAGING OFFLINE for None UAV Operator", "BLACK", 0.001];
						playSound "FD_CP_Not_Clear_F";
						waitUntil {sleep 0.1; currentVisionMode player != 2};
						LAYER cutText ["", "PLAIN"];
					};
