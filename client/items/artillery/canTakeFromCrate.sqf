...
switch (true) do
{
	case (isNull _crate): {};
	case (!alive player): {};
	case (_crate getVariable ["objectLocked", false] && !isNil {_crate getVariable "R3F_Side"} && (_crate getVariable ["R3F_Side", sideUnknown]) != playerSide && _crate getVariable ["OwnerUID", "0"] != getPlayerUID player): {};
