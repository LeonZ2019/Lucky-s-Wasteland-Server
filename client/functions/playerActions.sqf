...
[
	...
	["Take Uniform", "client\actions\takeUniform.sqf", [0], 100, true, true, "", "getItemCargo cursorObject select 0 findIf { _x isKindOf ['Uniform_Base', configfile >> 'CfgWeapons'] && !(player isUniformAllowed _x) } != -1 && cursorObject distance player <= 3 && (typeOf cursorObject == 'GroundWeaponHolder' || (cursorObject isKindOf 'ReammoBox_F' && !(cursorObject getVariable ['R3F_LOG_disabled', true]) && !(cursorObject getVariable ['objectLocked', false]) && ( cursorObject getVariable ['ownerUID', ''] == getPlayerUID player || (cursorObject getVariable ['R3F_Side', sideUnknown] == playerSide && cursorObject getVariable ['R3F_Side', sideUnknown] != INDEPENDENT) )))"], // from ground or crate
