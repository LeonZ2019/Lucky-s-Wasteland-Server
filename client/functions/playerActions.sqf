...
[
	...
	["<img image='client\icons\flat_grass.paa'/> Flatten Grass", "client\actions\flat_grass.sqf", [], 1, false, false, "", "vehicle player == player && !surfaceIsWater position player && insideBuilding player == 0"],
	...
	["Take Uniform", "client\actions\takeUniform.sqf", [0], 100, true, true, "", "getItemCargo cursorObject select 0 findIf { _x isKindOf ['Uniform_Base', configfile >> 'CfgWeapons'] && !(player isUniformAllowed _x) } != -1 && cursorObject distance player <= 3 && (typeOf cursorObject == 'GroundWeaponHolder' || (cursorObject isKindOf 'ReammoBox_F' && !(cursorObject getVariable ['R3F_LOG_disabled', true]) && (!(cursorObject getVariable ['objectLocked', false]) || (cursorObject getVariable ['objectLocked', false] && (cursorObject getVariable ['ownerUID', ''] == getPlayerUID player || (cursorObject getVariable ['R3F_Side', sideUnknown] == playerSide && cursorObject getVariable ['R3F_Side', sideUnknown] != INDEPENDENT))))))"], // from ground or crate
	...
	["<img image='\A3\ui_f\data\Map\VehicleIcons\pictureHeal_ca.paa'/> Treat yourself", "player action ['HealSoldierSelf', player];", [], 200, true, true, "", "!(player getUnitTrait 'Medic') && damage player <= 0.2 && damage player != 0 && 'FirstAidKit' in (items player) && count (['abswp', 'abdvp', 'advep', 'asdvp', 'asswp', 'aswmp'] select {[_x, animationState player] call fn_startsWith}) == 0 && vehicle player == player"],
...
if ((getDLCs 2) findIf { _x in [395180, 304380, 1021790] } != -1) then
{
	[player, ["<t color='#00FFFF'>Get in as Passenger</t>", "client\actions\moveInCargo.sqf", [], 6, true, true, "", "['LandVehicle', 'Air', 'Ship'] findIf { cursorTarget isKindOf _x} != -1 && (getText (configfile >> 'CfgVehicles' >> typeOf cursorTarget >> 'DLC')) in ['Expansion', 'Heli', 'Enoch', 'Orange'] && locked cursorTarget == 1 && (switch(getText (configfile >> 'CfgVehicles' >> typeOf cursorTarget >> 'DLC')) do {case'Expansion':{395180};case'Heli':{304380};case'Enoch':{1021790};case'Orange':{571710};}) in (getDLCs 2) && vehicle player == player && player distance cursorTarget < (3.4 max (sizeOf typeOf cursorTarget * 0.75)) && cursorTarget emptyPositions 'cargo' > 0"]] call fn_addManagedAction;
};
